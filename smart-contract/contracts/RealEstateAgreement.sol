...
contract RealEstateAgreement {
    ...
    struct ContractDetails {
        bytes32 dataHash; // 원본 데이터의 해시값
        bytes32 lessorSignatureHash; // 임대인의 사인패드 서명 값을 해시화한 값
        bytes32 tenantSignatureHash; // 임차인의 사인패드 서명 값을 해시화한 값
    }
    ...
    //매물 정보에 대한 해시값을 조회하는 함수
    function getContractDetails() public view returns (bytes32, bytes32, bytes32) {
        return (
            contractDetails.dataHash,
            contractDetails.lessorSignatureHash,
            contractDetails.tenantSignatureHash
        );
    }
    ...
    function sendContract(address _tenant, uint _rentAmount, uint _deposit, string memory _propertyAddress, string memory _specialTerms, string memory _lessorSignaturePad) public {
        ...
        // 계약서 내용을 해시화하여 dataHash 설정
        contractDetails.dataHash = hashContractDetails(_deposit, _rentAmount, _propertyAddress, _specialTerms, _lessorSignaturePad);
    ...
    function updateTenantSignaturePad(string memory _tenantSignaturePad) public onlyTenantOrLessor {
        ...
        // 임차인의 사인 패드 값을 해시화하여 tenantSignatureHash에 저장
        contractDetails.tenantSignatureHash = keccak256(abi.encodePacked(_tenantSignaturePad));
    ...
    function signContract(bytes memory _signature) public onlyTenantOrLessor {
        require(contractDetails.dataHash != 0, "The contract has not been initialized yet");

        // 계약서의 내용과 서명자에 따른 해시 값 계산(서명값 생성)
        bytes32 hash = keccak256(abi.encodePacked(contractDetails.dataHash, msg.sender == tenantPublicKey));
    ...
    function hashContractDetails(uint _deposit, uint _rentAmount, string memory _propertyAddress, string memory _specialTerms, string memory _lessorSignaturePad) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(_deposit, _rentAmount, _propertyAddress, _specialTerms, _lessorSignaturePad));
    }
